...
func Test_considerPropertyConditions(t *testing.T) {
	...
	matched, total, err := considerPropertyConditions(ac, res)
	assert.NoError(t, err)
	assert.Equal(t, 1, matched)
	assert.Equal(t, 1, total)
}

func Test_considerPropertyConditions_matched_12_from_20(t *testing.T) {
	ac := PropertyCondition{
		StringCondition: StringCondition{
			StringIn: map[string][]string{
				"prop:::employee:prop1": {"foo", "bar"},
...
func Test_considerPropertyConditions_error_props_not_found(t *testing.T) {
	ac := PropertyCondition{
		StringCondition: StringCondition{
			StringIn: map[string][]string{
				"prop:::employee:prop1": {"foo", "bar"},
	...
	_, _, err := considerPropertyConditions(ac, res)
	assert.Error(t, err)
}
